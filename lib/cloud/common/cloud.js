...
class Cloud extends chart_1.Chart {
    constructor() {
        super(...arguments);
        this.grid = [];
        this.pointsAtRadius = [];
    ...
    setConfig(config) {
        config = util_1.deepmerge({}, config_1.cloudConfig, this.config || {}, config || {});
    ...
    drawGridItems(grid, color = 'rgba(0, 0, 0, 0.2)') {
        grid.forEach((lines, i) => {
            lines.forEach((item, j) => {
                ...
                const node = new spritejs_1.Block({
                    bgcolor: color,
    ...
    updatePosition(width, height, pos) {
    ...
    async parseMaskGrid(pos) {
        const { image, text } = this.config.mask;
        if (image) {
            const imageData = await this.getImageMaskData(pos);
            return this.parseGridData(imageData);
        }
        else if (text) {
            const label = util_1.createLabel(text, this.config.mask);
            ...
            const imageData = { width: pos.width, height: pos.height, data: new Uint8ClampedArray() };
            return this.parseGridData(imageData);
        }
    }
    getGridItemStatus(imageData, row, column, gridSize, flag = false) {
        let index = 0;
    ...
    parseGridData(imageData, flag = false, gridSize = this.config.gridSize) {
    ...
    getImageOccupied(imageData, gridSize) {
    ...
    getPointsAtRadius(radius, center) {
